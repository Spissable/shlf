...
struct FileItemTests {
    ...
    func sortOrder() {
        let now = Date()
        let a = FileItem(url: URL(fileURLWithPath: "/a"), filename: "a", fileSize: 0, modificationDate: now.addingTimeInterval(-60))
        let b = FileItem(url: URL(fileURLWithPath: "/b"), filename: "b", fileSize: 0, modificationDate: now)
        let c = FileItem(url: URL(fileURLWithPath: "/c"), filename: "c", fileSize: 0, modificationDate: now.addingTimeInterval(-120))

        let sorted = [a, b, c].sorted { $0.modificationDate > $1.modificationDate }
        #expect(sorted.map(\.filename) == ["b", "a", "c"])
    }

    @Test("Relative date produces non-empty string")
    func relativeDate() {
        let item = FileItem(url: URL(fileURLWithPath: "/x"), filename: "x", fileSize: 0, modificationDate: Date().addingTimeInterval(-300))
